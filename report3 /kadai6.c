...
unsigned char readpict[PICTX][PICTY][3]; //画像読み込み用
char s[128];
int runx[PICTX][PICTY][2]; //ランレングス符号化x方向
int runy[PICTX][PICTY][2]; //ランレングス符号化y方向
int midx[64][2]; //位置検出パターン中心座標候補
int midy[64][2]; //位置検出パターン中心座標候補
int midnumx = 0;
int midnumy = 0;
...
for(int j = 0; j < PICTY; j++){
  for(int i = 0; i < PICTX; i++){
    lum = 0.298912 * readpict[i][j][0] + 0.586611 * readpict[i][j][1] + 0.114478 * readpict[i][j][2];
    lum = round(lum);
    hist[(int)lum] = hist[(int)lum] + 1;
  }
}
...
int num0 = 0; //黒クラスの画素数
int num1 = 0; //白クラスの画素数
int sum0 = 0; //黒クラスの輝度値合計
int sum1 = 0; //白クラスの輝度値合計
double ave0 = 0; //黒クラスの輝度値平均
double ave1 = 0; //白クラスの輝度値平均
double result = 0; //計算結果
double max = 0;
int maxt = 0; //求める閾値
for(int t = 0; t <= 255; t++){ //判別分析法
  num0 = 0;
  num1 = 0;
  sum0 = 0;
  sum1 = 0;

  for(int i = 0; i <= t; i++){ //黒クラス
    num0 += hist[i];
    sum0 += i * hist[i];
  }

  for(int i = 0; i <= t; i++){ //白クラス
    num1 += hist[i];
    sum1 += i * hist[i];
  }
  ...
  ave0 = (double)sum0 / (double)num0;
  ave1 = (double)sum1 / (double)num1;
  result = num0 * num1 * (ave0 - ave1) * (ave0 - ave1);
  if(result > max){
    max = result;
    maxt = t;
  }
...
for(int qrx = 0; qrx < WSYM; qrx++){
  for(int qry = 0; qry < WSYM; qry++){
    ...
    for(int a = 0; a < 3; a++){
      qrcode[qrx][qry][a] = readpict[(int)pictx][(int)picty][a];
    }
  }
}
/*--------------------シンボル復元終わり--------------------*/
